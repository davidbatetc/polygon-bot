...
def help(update, context):
    ...
    if len(context.args) > 1 or (len(context.args) == 1 and context.args[0] != 'program'):
        ...
        context.bot.send_message(
            ...
            'applied to `p` will affect `q` as well, and vice versa.\n'
            ...
            'and prints `no` otherwise.\n'
            '- `draw "image.png", p1, p2, p3, ...` creates a 400x400 image named '
            '"image.png" with the polygons `p1`, `p2`, `p3`, ...\n'
            ...
            'factor `k`. Note that in this case `p` has to be a variable.\n\n'
            '*Comments*\n'
